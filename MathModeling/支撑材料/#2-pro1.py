...
Outliers_to_drop = detect_outliers(dataframe, 2, ['OLR', 'F', 'inCOD', 'HLR', 'ALR', 'pH0', 'T', 'TSS', 'VFA', 'outCOD',
                                                  'CODrate', 'VFA1', 'pH1', 'T_in'])
print(Outliers_to_drop)
# Drop outliers

dataframe = dataframe.drop(Outliers_to_drop, axis=0).reset_index(drop=True)

dataframe.to_csv('result2.csv', index=False, sep=',')

#  线性相关性度量
print(dataframe.corr())
dataframe.corr().to_csv('corr2.csv', index=False, sep=',')

#  读取新的数据集文件，进行后续处理
file = open('result2.csv', encoding='UTF-8')
